...
public class VMController3 {
	...
	case RUNNING:
	    if (this.isAlive()) {
		    WriteFile.logger("\n ---> O Serviço foi iniciado!" , "VM3Controller_log.txt");
		    WriteFile.logger("Início. VM: " + "192.168.0.172","VM3Controller_log.txt");
		    WriteFile.logger(new Date().toString(), "VM3Controller_log.txt");
			...
			WriteFile.logger("Tempo de falha Gerado: " + waitingTime, "VMController_log.txt");
			WriteFile.logger(new Date().toString(), "VM3Controller_log.txt");
		    } else {
	...
	case TIMER_INJECT_FAILURE:
	    if (this.getTimer().isExpired()) {
			this.stopVM();
			WriteFile.logger("\n ---> O Serviço parou!" , "VM3Controller_log.txt");
			WriteFile.logger("Failed. VM: " + "192.168.0.172", "VM3Controller_log.txt");
			WriteFile.logger(new Date().toString(), "VM3Controller_log.txt");
	...
	case FAILED:
			...
			WriteFile.logger("Tempo de reparo Gerado: " + waitingTime, "VM3Controller_log.txt");
			WriteFile.logger(new Date().toString(), "VM3Controller_log.txt");
		    } else {

...
public class VMController6 {
    ...
    public VMController6(MySshConnector sshConnection, RandomVariateGenerator failure, RandomVariateGenerator repair) {
	...
	case RUNNING:
	    if (this.isAlive()) {
		    WriteFile.logger("\n ---> O Serviço foi iniciado!" , "VM6Controller_log.txt");
		    WriteFile.logger("Início. VM: " + "192.168.0.175","VM6Controller_log.txt");
		    WriteFile.logger(new Date().toString(), "VM6Controller_log.txt");
			...
			WriteFile.logger("Tempo de falha Gerado: " + waitingTime, "VM6Controller_log.txt");
			WriteFile.logger(new Date().toString(), "VM6Controller_log.txt");
		    } else {
	...
	case TIMER_INJECT_FAILURE:
	    if (this.getTimer().isExpired()) {
			this.stopVM();
			WriteFile.logger("\n ---> O Serviço parou!" , "VM6Controller_log.txt");
			WriteFile.logger("Failed. VM: " + "192.168.0.175", "VM6Controller_log.txt");
			WriteFile.logger(new Date().toString(), "VM6Controller_log.txt");
	...
	case FAILED:
			...
			WriteFile.logger("Tempo de reparo Gerado: " + waitingTime, "VM6Controller_log.txt");
			WriteFile.logger(new Date().toString(), "VM6Controller_log.txt");
		    } else {
